...
namespace {
    ...
    QString toolBarAreaStringFromDOMAttributes(const PHP::WriteInitialization::DomPropertyMap &attributes) {
...
void WriteInitialization::acceptUI(DomUI *node)
{
    ...
    m_output << m_option.indent << "public function " << "setupUi(/*" << widgetClassName << "*/ " << varName << ")\n"
           << m_option.indent << "{\n";
    ...
    m_output << "\n" << m_option.indent << "$this->retranslateUi(" << varName << ");\n";
    ...
    m_output << m_option.indent << "public function " << "retranslateUi(/*" << widgetClassName << "*/ " << varName << ")\n"
           << m_option.indent << "{\n"
...
void WriteInitialization::acceptWidget(DomWidget *node)
{
    ...
    {
        m_output << m_option.indent << "$this->" << m_driver->findOrInsertWidget(node) << " = new " << m_uic->customWidgetsInfo()->realClassName(className) << "($" << parentWidget << ");\n";
...
void WriteInitialization::acceptActionGroup(DomActionGroup *node)
{
    ...
    m_output << m_option.indent << "$this->" << actionName << " = new QActionGroup(" << varName << ");\n";
    m_output << m_option.indent << "$" << actionName << " = $this->" << actionName << "; //scope\n";
...
void WriteInitialization::acceptAction(DomAction *node)
{
    ...
    m_output << m_option.indent << "$this->" << actionName << " = new QAction(" << varName << ");\n";
    m_output << m_option.indent << "$" << actionName << " = $this->" << actionName << "; //scope\n";
...
void WriteInitialization::acceptActionRef(DomActionRef *node)
{
    ...
    m_actionOut << m_option.indent << varName << "->addAction($this->" << actionName << ");\n";
}
                                          ...
                                          unsigned flags)
{
    ...
    for (int i=0; i<lst.size(); ++i) {
        ...
        if (isTopLevel && propertyName == QLatin1String("geometry") && p->elementRect()) {
            ...
            m_output << m_option.indent << tempName << " = new QSize(" << w << ", " << h << ");\n"
        ...
        case DomProperty::Cstring:
            if (propertyName == QLatin1String("buddy") && m_uic->customWidgetsInfo()->extends(className, QLatin1String("QLabel"))) {
                ...
                if (stdset)
                    ...
                    propertyValue = QLatin1String("new QByteArray(");
                    propertyValue += fixString(p->elementCstring(), m_option.indent);
        ...
        case DomProperty::Cursor:
            propertyValue = QString::fromLatin1("new QCursor(static_cast<Qt::CursorShape>(%1))")
                            .arg(p->elementCursor());
            break;
        ...
        case DomProperty::LongLong:
            propertyValue = QLatin1String("new Q_INT64_C(");
            propertyValue += QString::number(p->elementLongLong());
            propertyValue += QLatin1Char(')');;
            break;
        case DomProperty::ULongLong:
            propertyValue = QLatin1String("new Q_UINT64_C(");
        ...
        case DomProperty::StringList:
            propertyValue = QLatin1String("new QStringList()");
