...
#endif
    } else if(option().generator == Option::CppGenerator) {
#ifdef QT_UIC_CPP_GENERATOR
...
#ifdef QT_UIC_PHP_GENERATOR
        if (!language.isEmpty() && language.toLower() != QLatin1String("php")) {
            fprintf(stderr, "uic: File is not a 'php' ui file, language=%s\n", qPrintable(language));
            return false;
        }

        rtn = pwrite (ui);
#else
        fprintf(stderr, "uic: option to generate php code not compiled in\n");
#endif
    }
